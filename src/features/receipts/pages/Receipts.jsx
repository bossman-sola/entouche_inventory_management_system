...
const MetricCard = ({
  ...
  >
    ...
    <div>
      <div style={{ color: "#6b7591", fontSize: 11, marginBottom: 3 }}>
        {label}
      </div>
      <div
        style={{
          fontSize: String(value).length > 8 ? 16 : 21,
          fontWeight: 700,
          lineHeight: 1.1,
        }}
      >
      ...
      <div
        ...
        style={{
          fontSize: 10.5,
          color: subAccent ? "#4f6ef7" : "#6b7591",
          marginTop: 2,
          fontWeight: subAccent ? 500 : 400,
          cursor: subAccent ? "pointer" : "default",
        }}
      >
        {sub}
      </div>
...
export default function Receipts() {
  ...
  const handleDeleteReceipt = async (receipt) => {
    if (
      !window.confirm(
        `Delete draft receipt ${receipt.no}? This can't be undone.`,
      )
    )
      return;
    setActioningId(receipt.id);
  ...
  const filtered = receipts.filter((r) => {
    ...
    if (supplierFilter !== "All Suppliers" && r.supplier !== supplierFilter)
      return false;
  ...
  const thisMonthLabel = now.toLocaleDateString("en-US", {
    month: "long",
    year: "numeric",
  });
  ...
  const handleExport = () => {
    const exportRows = filtered.map((r) => ({
      "Receipt Number.": r.no,
      "Receipt Date": r.date,
      "API PO Number": r.api_po_number,
      "PO Number from Vendor": r.po_number,
      Supplier: r.supplier,
      "Received By": r.by,
    ...
    >
      ...
      >
        <div>
          ...
          >
            View and manage all goods received into the warehouse
          </p>
          {actionError && (
            <div
              style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button
            ...
            style={{
              ...
              color: filtered.length === 0 ? "#b0b8cc" : "#1e2740",
              fontWeight: 500,
            }}
          >
      ...
      >
        ...
        <MetricCard
          ...
          value={fmtNaira(
            receipts
              .filter((r) => r.status !== "Cancelled")
              .reduce((a, r) => a + parseNaira(r.val), 0),
          )}
          sub="All time"
        />
        <MetricCard
      ...
      >
        ...
        <div style={{ position: "relative" }} ref={dateBtnRef}>
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            <span
              style={{ fontSize: 12, color: "#6b7591", whiteSpace: "nowrap" }}
            >
              Date Range
            </span>
            <div
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            >
              {["All Statuses", "Draft", "Received", "Cancelled"].map((s) => (
                <option key={s} value={s}>
                  {s}
                </option>
              ))}
                        ...
                        {openMenuId === r.id && menuPos && (
                          <div
                            ...
                            style={{
                              ...
                              border: "1px solid #e4e7ef",
                              borderRadius: 8,
                              boxShadow: "0 8px 24px rgba(20,25,50,0.14)",
                              minWidth: 170,
                              padding: 4,
                              textAlign: "left",
                            }}
