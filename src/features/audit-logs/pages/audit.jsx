...
function CalendarPicker({ value, onChange, onClose }) {
  ...
  const isStart = (d) =>
    d && localStart && d.toDateString() === localStart.toDateString();
  const isEnd = (d) =>
    d && localEnd && d.toDateString() === localEnd.toDateString();
  ...
  const apply = () => {
    onChange({ start: localStart, end: localEnd || localStart });
    onClose();
  };

  return (
    <div
      className="al-calendar"
      style={{
        position: "absolute",
        top: "calc(100% + 6px)",
        left: 0,
    ...
    >
      ...
      >
        ...
        <span style={{ fontWeight: 600, fontSize: 13.5, color: "#1e2740" }}>
          {MONTHS[month]} {year}
        </span>
        <button
          onClick={() => setViewDate(new Date(year, month + 1, 1))}
      ...
      >
        ...
        {days.map((d, i) => (
          <div
            key={i}
            onClick={() => handleDay(d)}
            onMouseEnter={() => d && selecting && setHover(d)}
              ...
              background:
                isStart(d) || isEnd(d)
                  ...
                  : inRange(d)
                    ...
                    : "transparent",
              color:
                isStart(d) || isEnd(d) ? "#fff" : d ? "#1e2740" : "transparent",
      ...
      >
        <div style={{ fontSize: 11.5, color: "#6b7591" }}>
          {localStart ? toStr(localStart) : "Start"}{" "}
          {localEnd ? `– ${toStr(localEnd)}` : ""}
        </div>
...
export default function AuditLogs() {
  ...
  const modulesPresent = useMemo(
    () => ["All Modules", ...Array.from(new Set(logs.map((l) => l.module)))],
    [logs],
  );
  const usersPresent = useMemo(
    () => ["All Users", ...Array.from(new Set(logs.map((l) => l.user)))],
    [logs],
  );
  ...
  useEffect(() => {
    if (!selected && logs.length) setSelected(logs[0]);
  }, [logs, selected]);
  useEffect(() => {
    const h = (e) => {
  ...
  const filtered = useMemo(() => {
    return logs.filter((l) => {
      ...
      const matchQ =
        ...
        l.fullDetails.toLowerCase().includes(q);
      ...
      const matchDate =
        !l.dateObj ||
        ((!dateRange.start || l.dateObj >= dateRange.start) &&
          (!dateRange.end ||
            l.dateObj <= new Date(dateRange.end.getTime() + 86399999)));
      return (
        matchQ && matchMod && matchAct && matchUsr && matchSts && matchDate
      );
    });
  ...
  const todaysCount = logs.filter(
    (l) => l.dateObj && l.dateObj.toDateString() === todayStr,
  ).length;
  ...
  const metrics = [
    {
      ...
      value: dataLoading ? "…" : logs.length,
      sub: "Derived from live records",
      iconBg: "#eef2ff",
      color: "#4f6ef7",
      icon: (
        <svg
    ...
    },
    {
      ...
      value: dataLoading ? "…" : criticalCount,
      sub: "Deleted records",
      iconBg: "#fff7ed",
      color: "#f59e0b",
      subAccent: true,
        ...
        >
          <div>
            ...
            <p style={{ color: "#6b7591", fontSize: 12.5, margin: "4px 0 0" }}>
              Activity reconstructed from live Items, Categories, Suppliers,
              Units and Users records - the API has no dedicated audit-log
              endpoint yet.
            </p>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {authStatus === "connecting" && (
              <span style={{ fontSize: 11.5, color: "#6b7591" }}>
                Connecting…
              </span>
            )}
            {isAuthed && (
              ...
              >
                ...
                />{" "}
                Live {dataLoading && "· syncing…"}
              </span>
            )}
            <button
              onClick={loadActivity}
              disabled={!isAuthed || dataLoading}
              title="Refresh"
              style={{
                width: 30,
                height: 30,
            ...
            >
              <svg
                ...
                strokeWidth="2"
                style={{
                  transform: dataLoading ? "rotate(180deg)" : "none",
        ...
        {authStatus === "error" && (
          ...
          >
            <span style={{ fontSize: 12.5, color: "#c0392b" }}>
              {authError || "Could not connect to the API."}
            </span>
            <button
            ...
            >
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div
                  style={{
                    fontSize: 11,
                    color: m.subAccent ? "#f59e0b" : "#6b7591",
                    fontWeight: m.subAccent ? 600 : 400,
                    whiteSpace: "nowrap",
                    overflow: "hidden",
                    textOverflow: "ellipsis",
                  }}
        ...
        <div className="al-layout">
          ...
          >
            <div className="al-toolbar">
              <div style={{ position: "relative", flex: "1 1 200px" }}>
                ...
                <input
                  ...
                  placeholder="Search by keyword, action, module or user..."
                  style={{
                    width: "100%",
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            <div style={{ color: "#9aa1b4", fontSize: 11.5 }}>
                              {log.time}
                            </div>
                          </td>
                          ...
                          <td style={{ padding: "12px 12px" }}>
                            <span className="al-link">{log.record}</span>
                          </td>
                          <td
                            style={{
                              padding: "12px 12px",
            ...
            >
              ...
              >
                <div className="al-pagbtn nav" onClick={() => setPage(1)}>
                  «
                </div>
                <div
                  className="al-pagbtn nav"
                ...
                {pageNums().map((n, i) =>
                  n === null ? (
                    <div key={"e" + i} className="al-pagbtn muted">
                      ...
                    </div>
                  ) : (
                    <div
                      key={n}
                      className={`al-pagbtn${n === page ? " active" : ""}`}
                      onClick={() => setPage(n)}
                    >
                      {n}
                    </div>
            ...
            >
              ...
              >
                {[
                  [
                    ...
                    <span
                      style={{
                        color:
                          (ACTION_COLORS[selected.action] || {}).color ||
                          "#4f6ef7",
                        fontWeight: 700,
                      }}
                    >
                  ...
                  [
                    "Record",
                    <span className="al-link">{selected.record}</span>,
                  ],
                  ...
                  >
                    <span style={{ fontSize: 12.5, color: "#6b7591" }}>
                      {label}
                    </span>
                    <span
                      style={{
                ...
                >
                  ...
                  <span style={{ fontSize: 12.5, color: "#9aa1b4" }}>
                    Not tracked by API
                  </span>
                </div>
              ...
              <div
                style={{ padding: "14px 18px", borderTop: "1px solid #e4e7ef" }}
              >
                <button
