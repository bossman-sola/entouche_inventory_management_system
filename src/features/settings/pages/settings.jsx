  ...
  return (
    <div
      ...
      style={{
        ...
        height: 24,
        borderRadius: 12,
        background: value ? "#4f6ef7" : "#d1d5e0",
        cursor: "pointer",
        position: "relative",
...
const Inp = ({ value, onChange, readOnly, placeholder }) => (
  <input
    ...
    style={{
      ...
      color: "#1e2740",
      outline: "none",
      background: readOnly ? "#f8f9fb" : "#fff",
      boxSizing: "border-box",
      transition: "border .15s",
    }}
    onFocus={(e) => {
      if (!readOnly) e.target.style.borderColor = "#4f6ef7";
    }}
...
const Sel = ({ value, onChange, options, disabled }) => {
  const normalizedOptions = options.map((option) => {
    if (typeof option === "string" && /^[A-Z]{3} \(/.test(option)) {
      return { value: option.slice(0, 3), label: option };
    }
    return option;
  });
  const selectedValue = normalizedOptions.some(
...
const SaveBtn = ({ onClick, saving }) => (
  <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 8 }}>
    <button
      ...
      style={{
        padding: "10px 28px",
        background: saving ? "#9aabf5" : "#4f6ef7",
        border: "none",
        borderRadius: 8,
        fontSize: 13.5,
        fontWeight: 600,
      ...
      onMouseEnter={(e) => {
        if (!saving) e.currentTarget.style.background = "#3a5be0";
      }}
      onMouseLeave={(e) => {
        if (!saving) e.currentTarget.style.background = "#4f6ef7";
      }}
    >
...
/* ─── Section Divider ─── */
const Divider = () => (
  <div style={{ borderTop: "1px solid #e4e7ef", margin: "28px 0" }} />
);
...
const NAV_ITEMS = [
  {
    key: "general",
    label: "General",
    sub: "Company, system and localization",
    icon: (
      <svg
        width={16}
        height={16}
  ...
  },
  {
    key: "warehouse",
    label: "Warehouse",
    sub: "Manage warehouses and locations",
    icon: (
      <svg
        width={16}
        height={16}
  ...
  },
  {
    key: "inventory",
    label: "Inventory",
    sub: "Stock, numbering and preferences",
    icon: (
      <svg
        width={16}
        height={16}
  ...
  },
  {
    key: "users",
    label: "Users & Roles",
    sub: "Default roles and permissions",
    icon: (
      <svg
        width={16}
        height={16}
  ...
  },
  {
    key: "integrations",
    label: "Integrations",
    sub: "Third-party services and API",
    icon: (
      <svg
        width={16}
        height={16}
  ...
  },
  {
    key: "security",
    label: "Security",
    sub: "Password, sessions and policies",
    icon: (
      <svg
        width={16}
        height={16}
  ...
  },
  {
    key: "audit",
    label: "Audit",
    sub: "Audit log and retention settings",
    icon: (
      <svg
        width={16}
        height={16}
...
export default function Settings() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      if (settingsRes.status === "fulfilled" && settingsRes.value) {
        // The API returns grouped arrays, e.g. { general: [{ key, value, type }] }.
        // Accept the legacy keyed-object format as well for backward compatibility.
        const flat = {};
        Object.values(settingsRes.value).forEach((group) => {
          const entries = Array.isArray(group)
            ? group.map((entry) => [entry.key, entry])
      ...
      if (auditRes.status === "fulfilled") {
        ...
        const count = Array.isArray(val)
          ? val.length
          : (val?.total ?? val?.data?.length ?? null);
  ...
  const warehouseOptions = useMemo(() => {
    ...
    return names.includes(defaultWarehouse)
      ? names
      : [defaultWarehouse, ...names];
  }, [warehouses]);

  const handleSaveSection = async (sectionKey) => {
    const fieldMap = SETTINGS_MAP[sectionKey];
    if (!fieldMap) {
      showToast("Nothing to save for this section");
      return;
    }
    const payload = {};
    Object.entries(fieldMap).forEach(([field, settingKey]) => {
  ...
  const actionItems = [
    {
      label: "Backup Now",
      sub: "Create a backup of your data now",
      color: "#4f6ef7",
      available: false,
      icon: (
        <svg
    ...
    },
    {
      ...
      sub: "Restore data from a previous backup",
      color: "#f59e0b",
      available: false,
      icon: (
        <svg
    ...
    {
      label: "Clear Cache",
      sub: "Improve system performance",
      color: "#8b5cf6",
      available: false,
      icon: (
        <svg
          ...
          <>
            ...
            >
              <SectionHead
                ...
                sub="Configure basic information about your organization and system."
              />
              <FieldRow cols="1fr 1fr 1fr">
                <Field label="Company Name">
                  <Inp value={companyName} onChange={setCompanyName} />
                </Field>
                <Field label="Company Email">
                  <Inp value={companyEmail} onChange={setCompanyEmail} />
                </Field>
                <Field label="Company Phone">
                  <Inp value={companyPhone} onChange={setCompanyPhone} />
                </Field>
              </FieldRow>
              <FieldRow cols="1fr 1fr 1fr">
                <Field label="Country">
                ...
                <Field label="Date Format">
                  <Inp value={dateFormat} onChange={setDateFormat} />
                </Field>
              </FieldRow>
              <FieldRow cols="1fr 1fr 1fr">
                <Field label="Time Format">
                  <Sel
                    ...
                    options={["12 Hour (hh:mm AM/PM)", "24 Hour (HH:mm)"]}
                  />
                </Field>
            ...
            >
              <SectionHead
                ...
                sub="Configure system wide preferences."
              />
              <div
                className="st-togglegrid"
                style={{
                  display: "grid",
              ...
              >
                {[
                  [
                    ...
                    setLowStockAlerts,
                    "Low Stock Alerts",
                    "Enable alerts for items below reorder level.",
                  ],
                  [
                    emailNotifications,
                  ...
                  [
                    requireReasonAdj,
                    setRequireReasonAdj,
                    "Require Reason on Adjustments",
                    "Users must provide reason for stock adjustments.",
                  ],
                  [
                    autoGenerateSku,
                    setAutoGenerateSku,
                    "Auto Generate Item SKU",
                    "Automatically generate SKU for new items.",
                  ],
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#9aa1b4", marginTop: 5 }}>
            ...
            >
              <SectionHead
                ...
                sub="Configure document prefixes and numbering."
              />
              <FieldRow cols="1fr 1fr 1fr 1fr 1fr">
                <Field label="Item Prefix">
                  <Inp value={itemPrefix} onChange={setItemPrefix} />
                </Field>
                <Field label="Inventory Prefix">
                  <Inp value={inventoryPrefix} onChange={setInventoryPrefix} />
                </Field>
                <Field label="Transfer Prefix">
                  <Inp value={transferPrefix} onChange={setTransferPrefix} />
                </Field>
                <Field label="Receipt Prefix">
                  <Inp value={receiptPrefix} onChange={setReceiptPrefix} />
                </Field>
                <Field label="Adjustment Prefix">
                  <Inp
                    value={adjustmentPrefix}
                    onChange={setAdjustmentPrefix}
                  />
                </Field>
              </FieldRow>
              <div style={{ marginBottom: 16 }}>
                <div
                  style={{ fontSize: 12, color: "#6b7591", marginBottom: 6 }}
                >
                  Numbering Reset
                </div>
                <div
                ...
                >
                  {["Do not reset", "Monthly", "Yearly"].map((opt) => (
                    <button
                      key={opt}
                      onClick={() => setNumberingReset(opt)}
                      style={{
                        padding: "7px 16px",
                        border: `1px solid ${numberingReset === opt ? "#4f6ef7" : "#e4e7ef"}`,
                        borderRadius: 7,
                        background: numberingReset === opt ? "#eef2ff" : "#fff",
                        color: numberingReset === opt ? "#4f6ef7" : "#1e2740",
                        fontSize: 12.5,
                        cursor: "pointer",
                        fontFamily: "inherit",
                        fontWeight: numberingReset === opt ? 600 : 400,
                      }}
                    >
                      {opt}
                    </button>
                  ))}
                  <span style={{ fontSize: 12, color: "#9aa1b4" }}>
                    Numbers will continue sequentially.
                  </span>
                </div>
              </div>
              <SaveBtn
        ...
        return (
          ...
          >
            <SectionHead
              ...
              sub="Manage warehouse and location preferences."
            />
            <FieldRow cols="1fr 1fr">
              ...
              <Field label="Warehouse Prefix">
                <Inp value={warehousePrefix} onChange={setWarehousePrefix} />
              </Field>
            </FieldRow>
            {warehouses.length === 0 && !loading && (
              ...
              >
                No warehouses returned by the API yet - showing the saved value
                only.
              </div>
            )}
            <div
            ...
            >
              {[
                [
                  enableBinLocations,
                  setEnableBinLocations,
                  "Enable Bin Locations",
                  "Allow items to be assigned to specific bin locations.",
                ],
                [
                  requireLocation,
                  setRequireLocation,
                  "Require Location on Receipt",
                  "Location must be specified when receiving items.",
                ],
              ].map(([val, setter, label, sub], i) => (
                <div
                  key={i}
            ...
            <SaveBtn
              onClick={() => handleSaveSection("warehouse")}
              saving={savingSection === "warehouse"}
            />
          </div>
        );
        ...
        return (
          ...
          >
            <SectionHead
              ...
              sub="Configure password policies, sessions and security options."
            />
            <FieldRow cols="1fr 1fr 1fr">
              ...
              <Field label="Lockout Duration (mins)">
                <Sel
        ...
        return (
          ...
          >
            <SectionHead
              ...
              sub="Configure email and system alert preferences."
            />
            <Field label="Notification Email">
              <Inp value={notifyEmail} onChange={setNotifyEmail} />
            </Field>
            <div style={{ height: 16 }} />
            <div
              className="st-togglegrid"
            ...
            >
              {[
                [
                  ...
                  "Receive emails when items fall below reorder level.",
                ],
                [
                  receiptAlerts,
                  setReceiptAlerts,
                ...
                [
                  ...
                  setTransferAlerts,
                  "Transfer Alerts",
                  "Notifications for inventory transfers.",
                ],
                [
                  dailyDigest,
                  setDailyDigest,
        ...
        return (
          ...
          >
            <SectionHead
              ...
              sub="Configure audit log and data retention settings."
            />
            {auditLogCount !== null && (
              <div
                style={{ fontSize: 12.5, color: "#6b7591", marginBottom: 16 }}
              >
                {auditLogCount} audit log entr
                {auditLogCount === 1 ? "y" : "ies"} currently recorded.
              </div>
            )}
            <div style={{ marginBottom: 16 }}>
            ...
            >
              {[
                [
                  ...
                  setLogLogins,
                  "Log User Logins",
                  "Record all login and logout events.",
                ],
                [
                  logDataChanges,
                  setLogDataChanges,
                  "Log Data Changes",
                  "Record all create, update and delete operations.",
                ],
                [
                  logExports,
                  setLogExports,
                  "Log Data Exports",
                  "Record all data export and download events.",
                ],
                [
                  autoDeleteLogs,
                  setAutoDeleteLogs,
    ...
    >
      ...
      <div className="st-page" style={{ paddingBottom: 0 }}>
        <div style={{ marginBottom: 24 }}>
          ...
          <p style={{ color: "#6b7591", fontSize: 12.5, margin: "4px 0 0" }}>
        ...
        >
          <span style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontSize: 13.5, fontWeight: 600, color: "#1e2740" }}>
              {activeNavItem?.label}
            </span>
          </span>
          <svg
        ...
        <div className="st-layout">
          ...
          >
            ...
            >
              ...
              >
                <span
                  style={{ fontWeight: 700, fontSize: 13.5, color: "#1e2740" }}
                >
                  Company Information
                </span>
              </div>
              <div style={{ padding: "18px 18px 6px" }}>
                ...
                <div style={{ textAlign: "center", marginBottom: 16 }}>
                  <div
                    style={{ fontWeight: 700, fontSize: 14, color: "#1e2740" }}
                  >
                    {companyName || "Not set"}
                  </div>
                  <div style={{ fontSize: 12, color: "#9aa1b4", marginTop: 3 }}>
            ...
            >
              ...
              {[
                [
                  "Audit Log Entries",
                  auditLogCount !== null ? String(auditLogCount) : "-",
                ],
              ...
              <div style={{ fontSize: 11, color: "#c9cedd", marginTop: 8 }}>
                Version, environment and backup info aren't exposed by the API
                yet.
              </div>
            </div>
                ...
                {actionItems.map((a, i) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: "#9aa1b4" }}>
                        {a.available
                          ? a.sub
                          : "Not available - no matching endpoint"}
                      </div>
                    </div>
                  </button>
                ))}
