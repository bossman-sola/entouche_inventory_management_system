...
import { downloadImportTemplate, ApiError } from "../../../lib/api";

function downloadLocalTemplate(name) {
...
export function ImportTemplates() {
  ...
  const handleDownload = async (name) => {
    ...
    if (name === "Inventory Template") {
      try {
        await downloadImportTemplate("inventory");
      } catch (err) {
        setDownloadError(
          err instanceof ApiError ? err.message : "Could not download the inventory template."
        );
      }
      return;
    }

    downloadLocalTemplate(name);
  };
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-5">
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        {TEMPLATES.map(t => (
          <button key={t.name} onClick={() => handleDownload(t.name)}
      ...
      {downloadError && <p className="text-xs text-red-500 mt-3">{downloadError}</p>}
    </div>
  );
}